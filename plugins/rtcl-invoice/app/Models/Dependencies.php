...
namespace RtclInvoice\Models;

use Rtcl;

class Dependencies {
	const MIN_RTCL = '2.3.7';
	...
	public function _missing_plugins_warning() {
		...
		foreach ( $this->missing as $title => $url ) {
			...
			if ( $counter == sizeof( $this->missing ) ) {
				...
				$sep = ' ' . esc_html__( 'and', 'rtcl-invoices' ) . ' ';
			} else {
				$sep = ', ';
			}
        ...
        <div class="notice notice-error">
            <p>
				<?php echo wp_kses( sprintf( __( '<strong>Classified Listing – PDF Invoices</strong> is enabled but not effective. It requires %s in order to work.', 'rtcl-invoices' ), $missing ),
					[
						'strong' => [],
						'a'      => [
							'href'  => true,
	...
	public function _old_rtcl_warning() {
		...
		$message = wp_kses( __( sprintf( '<strong>Classified Listing – PDF Invoices</strong> is enabled but not effective. It is not compatible with <a class="thickbox open-plugin-details-modal" href="%1$s">Classified Listing</a> versions prior %2$s',
			$link,
			self::MIN_RTCL
		), 'rtcl-invoices' ), [ 'strong' => [], 'a' => [ 'href' => true, 'class' => true ] ] );
