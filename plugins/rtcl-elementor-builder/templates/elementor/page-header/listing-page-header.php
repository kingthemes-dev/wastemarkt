...
?>
	<div class="rtcl el-single-addon rtcl-el-listing-page-header header-inner-wrapper header-<?php echo ! empty( $instance['rtcl_header_style'] ) ? esc_attr( $instance['rtcl_header_style'] ) : ''; ?>">
		<?php if ( ! empty( $instance['rtcl_show_page_title'] ) ) { ?>
			<header class="rtcl-listing-header">
				...
				printf( '<%1$s class="rtcl-listings-header-title page-title %3$s" >%2$s</%1$s>',
					esc_html( Utils::validate_html_tag( $instance['header_size'] ) ), esc_html( $title_text ),
					'text-' . esc_attr( $instance['title_alignment'] ) );
				?>
				<?php
				if ( Fns::is_builder_page_archive() ) {
					/**
