...
<div <?php Functions::listing_class( [ 'rtcl-widget-listing-item', $item_class, 'listing-item', $img_position_class ] ); ?>>
	...
	if ( rtcl()->has_pro() ) {
		if ( ! empty( $instance['rtcl_show_quick_view'] ) ) :
			...
			<div class="rtcl-el-button">
				<a class="rtcl-quick-view" href="#" title="<?php esc_attr_e( 'Quick View', 'rtcl-elementor-builder' ); ?>"
	...
	if ( rtcl()->has_pro() ) {
		if ( ! empty( $instance['rtcl_show_compare'] ) ) :
			...
			<div class="rtcl-el-button">
				...
				?>
				<a class="rtcl-compare <?php echo esc_attr( $selected_class ); ?>" href="#" title="<?php esc_attr_e( 'Compare', 'rtcl-elementor-builder' ); ?>"
				   data-listing_id="<?php echo absint( $_id ); ?>">
	...
	$button               = sprintf( '<div class="rtcl-meta-buttons-wrap %s-layout meta-button-count-%s">%s %s %s</div>', $action_button_layout, $button_icon,
		$dispaly_favourites, $dispaly_quick_view, $dispaly_compare );
	...
	if ( $instance['rtcl_show_user'] ) {
		...
		$author_html       = sprintf( '<li class="author" ><i class="rtcl-icon rtcl-icon-user" aria-hidden="true"></i>%s %s</li>', get_the_author(),
			$after_author_meta );
	}
	$views_html = '';
	...
	ob_start();
	do_action( 'rtcl_builder_loop_item_content_end' );
	$hook_content_end = ob_get_clean();

	$item_content = sprintf(
		'<div class="item-content">%s %s %s %s %s %s %s %s</div>%s',
		$labels,
		$category,
		$listing_title,
		$custom_field,
	...
	Functions::print_html( $final_contents, true );
	?>
