...
$total_comments = !get_the_ID() ? 2 : get_option( 'comments_per_page' );
...
$classes[] = ! empty( $instance['rtcl_show_comment_list'] ) && $instance['rtcl_show_comment_list'] == 'on'  ? 'displayed-comment-list' : 'no-comment-list';
$classes[] = ! empty( $instance['rtcl_show_comment_form'] ) &&  $instance['rtcl_show_comment_form'] == 'on' ? 'displayed-comment-form' : 'no-comment-form';
?>
<div class="rtcl">
	<div class="el-single-addon rtcl-Reviews rtcl <?php echo esc_attr( implode( ' ', $classes ) ); ?>">
		<?php if ( $instance['rtcl_show_comment_list'] === 'on' ) { ?>
			<div id="comments">
				<?php
				$has_header_content = false;
				if (
					$instance['rtcl_show_review_title'] === 'on' ||
					 $instance['rtcl_review_meta'] === 'on' ||
					 $instance['rtcl_show_leave_review_button'] === 'on'
				) {
				...
				<?php if ( $has_header_content ) { ?>
					<div class="rtcl-reviews-meta">
						<?php if ( ! empty( $instance['rtcl_show_review_title'] ) && $instance['rtcl_show_review_title'] === 'on' ) { ?>
							<h4 class="rtcl-single-listing-section-title">
								<?php echo esc_html( $instance['rtcl_review_title_text'] ); ?>
							</h4>
						<?php } ?>
						<?php
						if ( count( $comments ) && ! empty( $instance['rtcl_review_meta'] ) && $instance['rtcl_review_meta'] === 'on' ) {
						...
						?>
						<?php if ( ! empty( $instance['rtcl_show_leave_review_button'] ) && $instance['rtcl_show_leave_review_button'] === 'on' ) { ?>
		...
		<?php } ?>
		<?php if ( $instance['rtcl_show_comment_form'] && $instance['rtcl_show_comment_form'] === 'on'  ) { ?>
			<div id="review-form-wrapper">
				<div id="review-form">
					...
					$submit_btn_text = !empty($instance['rtcl_comment_form_button_text']) ? esc_html($instance['rtcl_comment_form_button_text']) : esc_html__('Submit Review', 'rtcl-elementor-builder' );
					$commenter          = wp_get_current_commenter();

					$comment_form     = [
						// translators: %s: Listing Litle
						'title_reply'         => $comment_form_title ? $comment_form_title : sprintf( __( 'Be the first to review &ldquo;%s&rdquo;', 'rtcl-elementor-builder' ), get_the_title() ),
					...
					$comment_form['comment_field'] = '<div class="comment-form-title  rtcl-form-group"><label for="title">' . esc_html__( 'Review title', 'rtcl-elementor-builder' ) . '&nbsp;<span class="required">*</span></label><input type="text" class="rtcl-form-control" name="title" id="title"  aria-required="true" required/></div>';
