...
?>
	...
	<div <?php Functions::listing_class( [ 'rtcl-widget-listing-item', 'listing-item swiper-slide-customize', $img_position_class ] ); ?>>
		...
		if ( $instance['rtcl_show_labels'] === 'on' ) {
			$labels = $listing->badges() ? $listing->badges() : '';
		}
		if ( $instance['rtcl_show_date'] === 'on' ) {
		...
		if ( $instance['rtcl_show_location'] === 'on' ) {
		...
		if ( $instance['rtcl_show_category'] === 'on' ) {
		...
		$views_html = '';
		if ( $instance['rtcl_show_views'] === 'on' ) {
		...
		if ( $instance['rtcl_show_types'] === 'on' && $listing->get_ad_type() ) {
		...
		if ( $instance['rtcl_show_title'] === 'on' ) {
		...
		if ( $instance['rtcl_show_description'] === 'on' ) {
		...
		ob_start();
		?>
		<?php if ( Functions::is_enable_favourite() || $instance['rtcl_show_user'] === 'on' ) : ?>
			<div class="rtin-bottom  <?php echo ! empty( $instance['rtcl_show_user'] ) ? 'has-author' : ''; ?>">
				<ul>
					<?php if ( $instance['rtcl_show_user'] === 'on' ) : ?>
						<li class=" action-btn item-author">
							...
							<?php
							if( $instance['rtcl_verified_user_base'] === 'on' ){
								do_action('rtcl_after_author_meta', $listing->get_owner_id() );
							}
							?>
						</li>
					<?php endif; ?>
					<?php if ( Functions::is_enable_favourite() && $instance['rtcl_show_favourites'] === 'on' ) : ?>
						<li class="action-btn">
							<?php
					...
					<?php
					if ( rtcl()->has_pro() ) {
						if (  $instance['rtcl_show_quick_view'] === 'on' ) :
							?>
							<li class="action-btn">
					...
					<?php
					if ( rtcl()->has_pro() ) {
						if (  $instance['rtcl_show_compare'] === 'on' ) :
							?>
							<li class="action-btn">
								<?php
		...
		if ($instance['rtcl_show_custom_fields'] === 'on') {
			ob_start();
			if ( rtcl()->has_pro() ) {
