		...
		?>
				<div class="rtin-head-area">
					...
					if ( ! empty( $settings['rtcl_show_count'] ) ) {
						$text = absint( $count ) > 1 ? $settings['rtcl_count_text_plural'] : $settings['rtcl_count_text'];
						printf( "<div class='views'>%d <span class='ads-count'></span>%s</span></div>", absint( $count ), $text ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
					?>
				</div>
				<div class="box-body">
					...
					if ( $settings['rtcl_show_sub_category'] && ! empty( $child_terms ) && ! is_wp_error( $child_terms ) ) {
						?>
							...
							<?php foreach ( $child_terms as $child_trm ) { ?>
										<li>
											<a href="<?php echo esc_url( get_term_link( $child_trm ) ); ?>"> <i class="rtcl-icon rtcl-icon-angle-right"></i> <span><?php echo esc_html($child_trm->name) . ' (' . esc_attr($child_trm->count) . ')'; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped?></span> </a>
										</li>
								<?php } ?>
							</ul>
						<?php
				...
				<?php if ( $settings['rtcl_description'] && $trm->description ) { ?>
					<div class="description-section">
						<?php
							$word_limit = $settings['rtcl_content_limit'] ? wp_kses_post(wp_trim_words( $trm->description, $settings['rtcl_content_limit'] ) ): wp_kses_post($trm->description);
