...
<div id="reviews_" class="rtcl-Reviews rtcl">
	<div id="comments">
		<div class="rtcl-reviews-meta">
			<div class="rtcl-section-heading-simple">
				<h3><?php esc_html_e( "Reviews", "classified-listing-pro" ); ?></h3>
			</div>
	...
	<div id="review-form-wrapper">
		<div id="review-form">
			...
			$comment_form = [
				'title_reply'         => have_comments() ? esc_html__( 'Leave Review', 'classified-listing-pro' )
					: sprintf( __( 'Be the first to review &ldquo;%s&rdquo;', 'classified-listing-pro' ), get_the_title() ),
				'title_reply_to'      => __( 'Leave a Reply to %s', 'classified-listing-pro' ),
				'title_reply_before'  => '<h4 id="reply-title" class="comment-reply-title">',
				'title_reply_after'   => '</h4>',
				'comment_notes_after' => '',
				'fields'              => [
					'author' => '<div class="comment-form-author rtcl-form-group">' . '<label for="author">' . esc_html__( 'Name', 'classified-listing-pro' )
								. '&nbsp;<span class="required">*</span></label> ' .
								'<input id="author" class="rtcl-form-control" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] )
								. '" size="30" aria-required="true" required /></div>',
					'email'  => '<div class="comment-form-email rtcl-form-group"><label for="email">' . esc_html__( 'Email', 'classified-listing-pro' )
								. '&nbsp;<span class="required">*</span></label> ' .
								'<input id="email" name="email" class="rtcl-form-control" type="email" value="' . esc_attr( $commenter['comment_author_email'] )
								. '" size="30" aria-required="true" required /></div>',
				],
				'label_submit'        => esc_html__( 'Submit', 'classified-listing-pro' ),
				'class_submit'        => 'rtcl-btn rtcl-btn-primary',
				'logged_in_as'        => '',
				'comment_field'       => '',
			];
			...
			if ( $account_page_url = Link::get_my_account_page_link() ) {
				$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a review.',
						'classified-listing-pro' ), esc_url( $account_page_url ) ) . '</p>';
			}

			$comment_form['comment_field'] = '<div class="comment-form-title rtcl-form-group"><label for="title">' . esc_html__( 'Review title',
					'classified-listing-pro' )
											 . '&nbsp;<span class="required">*</span></label><input type="text" class="rtcl-form-control" name="title" id="title"  aria-required="true" required/></div>';
			if ( Functions::get_option_item( 'rtcl_moderation_settings', 'enable_review_rating', false, 'checkbox' ) ) {
				$comment_form['comment_field'] .= '<div class="comment-form-rating rtcl-form-group"><label for="rating">' . esc_html__( 'Your rating',
						'classified-listing-pro' ) . '<span class="required">*</span></label><select name="rating" id="rating" class="rtcl-form-control" aria-required="true" required>
			...
			$comment_form['comment_field'] .= '<div class="comment-form-comment rtcl-form-group"><label for="comment">' . esc_html__( 'Your review',
					'classified-listing-pro' )
											  . '&nbsp;<span class="required">*</span></label><textarea id="comment" class="rtcl-form-control" name="comment" cols="45" rows="8" aria-required="true" required></textarea></div>';
