...
class StripePaymentGateway extends PaymentGateway {
	...
	public function validate_minimum_order_amount( $order ) {
		if ( $order->get_total() * 100 < StripeHelper::get_minimum_amount() ) {
			...
			throw new StripeException( 'Did not meet minimum amount', sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'classified-listing-pro' ), Functions::price( StripeHelper::get_minimum_amount() / 100 ) ) );
		}
	}
	...
	public function generate_create_intent_request( $order, $prepared_obj ) {
		...
		if ( isset( $full_request['statement_descriptor'] ) ) {
			$request['statement_descriptor_suffix'] = $full_request['statement_descriptor'];
		}
