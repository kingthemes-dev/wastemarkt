...
class Api {
	 ...
	 */
	static function is_valid_http_auth_request( $request, $sessionData = [] ) {
		...
		rtcl()->session->set( 'rtcl_app_web_view', true );

		if ( !empty( $sessionData ) && is_array( $sessionData ) ) {
			foreach ( $sessionData as $sKey => $sValue ) {
				rtcl()->session->set( $sKey, $sValue ?: true );
			}
		}
	...
	static function get_listing_data( $listing, $as_top = false ) {
		...
		$data = [
			...
			'author'        => self::get_user_data( get_user_by( 'ID', $listing->get_owner_id() ) ),
			...
			'tags'          => get_the_terms( $listing->get_id(), rtcl()->tag ),
	...
	static function get_order_data( $order_id ) {
		...
		$order_data = [
			...
			"status"         => str_replace( 'rtcl-', '', $order->get_status() ),
	...
	public static function get_single_listing_data( Listing $listing ) {
		...
		if ( FBHelper::isEnabled() ) {
			$data['fb_custom_fields'] = self::get_fb_custom_fields( $listing );
			$data['form_data'] = FBHelper::getFormData( $listing->get_id(), $listing->getForm() );
		} else {
	 ...
	 * @return array
	 */
	static function get_fb_custom_fields( Listing $listing ): array {
	 ...
	 * @return array
	 */
	static function get_custom_fields( $category_id = 0, $post_id = 0, $request = [] ) {
		$fields = [];
		if ( FBHelper::isEnabled() ) {
			...
			$directory = !$directory
				? ''
				: ( $directory === 'all' ? 'all'
					: ( is_numeric( $directory ) ? absint( $directory ) : '' ) );
			$directoryData = FBHelper::getDirectoryData( $directory );
			$current_term = get_term_by( 'term_id', $category_id, rtcl()->category );
			$catIds = !is_wp_error( $current_term ) ? $current_term->term_id : '';
			$filterTypes = [
				'text',
				'textarea',
				'number',
