...
{
    const MIN_RTCL = '3.1.6';
    const MIN_RTCL_PRO = '2.3.9';
    ...
    public function _missing_plugins_warning() {
        ...
        foreach ($this->missing as $title => $url) {
            ...
            if ($counter == sizeof($this->missing)) {
                ...
                $sep = ' ' . __('and', 'rtcl-multi-currency') . ' ';
            } else {
                $sep = ', ';
            }
        ...
        <div class="message error">
            <p><?php echo wp_kses(sprintf(__('<strong>Classified Listing – Multi Currency</strong> is enabled but not effective. It requires %s in order to work.', 'rtcl-multi-currency'), $missing), ['strong' => [], 'a' => ['href' => true, 'class' => true]]); ?></p>
        </div>
        <?php
    }
    ...
    public function _old_rtcl_warning() {
        ...
        $message = wp_kses(__(sprintf('<strong>Classified Listing – Multi Currency</strong> is enabled but not effective. It is not compatible with <a class="thickbox open-plugin-details-modal" href="%1$s">Classified Listing</a> versions prior %2$s.',
            $link,
            self::MIN_RTCL
        ), 'rtcl-multi-currency'), ['strong' => [], 'a' => ['href' => true, 'class' => true]]);

        printf('<div class="notice notice-error"><p>%1$s</p></div>', $message);
    }

    public function _old_rtcl_pro_warning() {
        $message = wp_kses(__(sprintf('<strong>Classified Listing – Multi Currency</strong> is enabled but not effective. It is not compatible with <a href="%1$s">Classified Listing pro</a> versions prior %2$s.',
            esc_url('https://www.radiustheme.com/downloads/classified-listing-pro-wordpress/'),
            self::MIN_RTCL_PRO
        ), 'rtcl-multi-currency'), ['strong' => [], 'a' => ['href' => true]]);
