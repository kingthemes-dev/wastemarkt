...
//		Functions::add_default_expiry_date( $listing->get_id() );
        ...
        if ( get_post_meta( $listing->get_id(), 'never_expires', true ) ) {
            $expire_at = esc_html__( 'Never Expires', 'classified-listing-store' );
    ...
    public static function rtcl_ajax_store_banner_upload() {
        ...
        if ( isset( $_FILES['banner'] ) ) {
            ...
            if ( $status && !isset( $status['error'] ) ) {
                ...
                $new_store_status = Functions::get_option_item( 'rtcl_membership_settings', 'new_store_status', 'publish' );

                // Create a post if does not exist
                if ( $store_id < 1 ) {
                ...
                if ( !is_wp_error( $attach_id ) ) {
                    ...
                    wp_update_attachment_metadata( $attach_id, Functions::generate_attachment_metadata( $attach_id, $filename, StoreFunctions::get_store_banner_sizes() ) );
        ...
        if ( isset( $_FILES['logo'] ) ) {
            ...
            if ( $status && !isset( $status['error'] ) ) {
                ...
                if ( !is_wp_error( $attach_id ) ) {
                    ...
                    wp_update_attachment_metadata( $attach_id, Functions::generate_attachment_metadata( $attach_id, $filename, StoreFunctions::get_store_Logo_sizes() ) );
	                set_post_thumbnail( $store_id, $attach_id );
        ...
        if ( Functions::verify_nonce() ) {
            ...
            if ( $title && ( ( isset( $data['id'] ) && $slug ) || ( !isset( $data['id'] ) && !$slug ) ) ) {
                ...
                if ( $store = RtclFunctions::get_current_user_store() ) {
                    ...
                    $store_arg['post_status'] = $new_store_status;
