...
<div class="rtcl rtcl-listings-sc-wrapper rtcl-divi-element rtcl-elementor-widget ">
		...
		<div class="rtcl-listings <?php echo esc_attr( $class ); ?> ">
			...
			if ( rtcl()->has_pro() && Fns::is_enable_top_listings() && $top_query->have_posts() ) {
				...
				while ( $top_query->have_posts() ) :
					$top_query->the_post();
					$top_data = [
						'template'              => 'divi/listing-archive/' . $view . '/' . $style,
						'instance'              => $instance,
						'style'                 => $style,
						'item_class'            => 'as-top',
						'default_template_path' => \RtclElb\Helpers\Fns::get_plugin_template_path(),
					];
					...
					do_action( 'rtcl_builder_loop_item_end' );
				endwhile;
				wp_reset_postdata();
			}
...
//		do_action( 'rtcl_after_listing_loop' );
		...
		if (  $instance['rtcl_listing_pagination']   ) {
