<?php

namespace RtclElb\Blocks;

use RtclElb\Abstracts\BlockBase;
use Rtcl\Helpers\Functions;
use RtclElb\Traits\BlockSingleListingTraits;
use RtclElb\Traits\Singleton;

class ListingImage extends BlockBase
{
	use Singleton;
	use BlockSingleListingTraits;

	public function __construct()
	{
		parent::__construct();
		add_action('template_redirect', [$this, 'set_listing']);
	}

	public function block_attributes()
	{
		$attributes = [
			'showLightboxIcon' => array(
				'type'    => 'boolean',
				'default' => true
			),
			'enableZoom' => array(
				'type'    => 'boolean',
				'default' => true
			),
			'enableSlider' => array(
				'type'    => 'boolean',
				'default' => true
			),
			'showBadge' => array(
				'type'    => 'boolean',
				'default' => true
			),
			'showVideo' => array(
				'type'    => 'boolean',
				'default' => true
			),
			'showArrow' => array(
				'type'    => 'boolean',
				'default' => true
			),
			'enableThumbSlider' => array(
				'type'    => 'boolean',
				'default' => true
			),
			"galleryImageSize" => array(
				"type" => "string",
				"default" => "rtcl-gallery",
			),
			'arrowColor' => array(
				'type'    => 'string',
				'default' => ''
			),
			'arrowBgColor' => array(
				'type'    => 'string',
				'default' => ''
			),
			'zoomIconColor' => array(
				'type'    => 'string',
				'default' => ''
			),
			'zoomIconBgColor' => array(
				'type'    => 'string',
				'default' => ''
			),

		] + parent::common_attributes();

		return apply_filters('rtcl_listing_image_block_attributes', $attributes);
	}

	public function register_block()
	{
		register_block_type(
			RTCL_ELB_PATH . 'app/Blocks/listing-image',
			[
				'render_callback' => [$this, 'render_block'],
				'attributes'      => $this->block_attributes(),
			]
		);
	}

	public function render_block($attributes)
	{
		$this->attributes = $attributes;
		$template_style = 'listing-single/listing-image';
		$data = [
			'template'              => $template_style,
			'settings'              => $attributes,
			'listing'               => $this->listing,
			'default_template_path' => rtclElb()->get_plugin_block_template_path(),
		];
		$data  = array_merge(
			$data,
			$this->get_the_gallery($this->listing)
		);
		$data = apply_filters('rtcl_block_listing_image_data', $data);
		ob_start();
		Functions::get_template($data['template'], $data, '', $data['default_template_path']);
		return ob_get_clean();
	}
}
