...
use RtclElb\Controllers\Hooks\DiviHooks;
...
class DiviController {
	...
	public function init() {
		...
		add_action( 'admin_enqueue_scripts', [ __CLASS__, 'rtcl_reb_et_builder_enqueue_assets_main' ], 999999 );
		$this->hooks();
	}
	...
	public static function rtcl_reb_et_builder_enqueue_assets_main() {
			...
			li.et_fb_classified_store_single_page_modules.et_pb_folder:before,
			li.et_fb_classified_general_module.et_pb_folder:before,
			li.et_fb_classified_single_page_modules.et_pb_folder:before,
			li.et_fb_classified_Archive_modules.et_pb_folder:before {
			    content: "" !important;
...
CSS;
	...
	public static function front_end_script() {
		wp_register_style( "rtcl-divi-addons-builder", rtclElb()->assets_url( 'css/divi-element.css' ), [ 'rtcl-public' ], RTCL_ELB_VERSION );
//		wp_register_script( "rtcl-divi-addons-builder", CLASSIFIED_LISTING_TOOLKITS_BUILD . "/divi-frontend-element.js", [ 'jquery' ], CLASSIFIED_LISTING_TOOLKITS_VERSION, true );

		wp_enqueue_style( "rtcl-divi-addons-builder" );

		wp_enqueue_script( 'rtcl-divi-modules-builder', rtclElb()->assets_url( 'js/divi-modules.js' ),
			[ 'jquery', 'react-dom', 'react', 'et_pb_media_library', 'wp-element', 'wp-i18n' ],
			RTCL_ELB_VERSION, true );

		$localize = [
			'rtcl_nonce'            => wp_create_nonce( 'rtcl-nonce' ),
			'is_divi_plugin_active' => Fns::is_divi_plugin_active(),
			'get_the_title'         => Fns::get_the_title()
		];

		wp_localize_script( 'rtcl-divi-modules-builder', 'rtcl_divi', $localize );
	}
