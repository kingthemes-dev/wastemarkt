        ...
        if (!empty($listings)) {
			...
			if ( ! empty( $custom_field_ids ) ) {
				...
				foreach ( $custom_field_ids as $custom_field_id ) {
					$cf = rtcl()->factory->get_custom_field( $custom_field_id );
					if ( ! $cf ) {
						continue;
					}
					$cFieldValues = [];
					...
					foreach ( $listings as $listing ) {
						$_value = $cf->getFormattedCustomFieldValue( $listing->get_id() );
						$_value = $cFieldValues[ $listing->get_id() ] = is_array( $_value ) && empty( $_value ) ? '' : $_value;
						if ( $_value ) {
							$loop = true;
						}
					}
					...
					<tr class="rtcl-compare-table-item" data-item="_cf_<?php echo absint( $custom_field_id ) ?>">
						<th class="rtcl-compare-table-label"><?php echo esc_html( $cf->getLabel() ); ?></th>
						<?php
						foreach ( $cFieldValues as $listingId => $cFieldValue ) {
							?>
							<td class="rtcl-compare-table-value"
								data-value-item="_cf_<?php echo absint( $custom_field_id ) ?>"
								data-listing_id="<?php echo $listingId; ?>">
								<?php if ( ! empty( $cFieldValue ) ) {
									Functions::print_html( $cFieldValue );
								} else {
									echo '--';
								}; ?>
							</td>
