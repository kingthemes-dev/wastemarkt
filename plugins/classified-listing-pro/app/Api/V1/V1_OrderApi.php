...
class V1_OrderApi {
	public function register_routes() {
		...
		register_rest_route( 'rtcl/v1', 'get-checkout-data', [
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => [ $this, 'checkout_data_callback' ],
			'permission_callback' => [ Api::class, 'permission_check' ]
		] );
	...
	public function checkout_callback( WP_REST_Request $request ) {
		...
		if ( !$gateway && $plan && ( $plan->getPrice() + 0 ) === 0 ) {
		...
		if ( !empty( $billingFields ) ) {
			foreach ( $billingFields as $_key => $field ) {
				if ( $request->get_param( $_key ) && !isset( $_POST[$_key] ) ) {
					$_POST[$_key] = $request->get_param( $_key );
				}
		...
		if ( "promotion" === $type && 'regular' === $plan->getType() ) {
			$metaInputs['listing_id'] = $listing ? $listing->get_id() : 0;
		}
		...
		try {
			...
			$response = [
				'status'  => "error",
				'error'   => 'PAYMENT_PROCESS_ERROR',
				'code'    => '403',
				'message' => $e->getMessage() ?: esc_html__( "Error to process the payment.", "classified-listing-pro" )
			];
			wp_send_json( $response, 403 );
		}
	 ...
	 * @return WP_Error|WP_HTTP_Response|WP_REST_Response
	 */
	public function checkout_data_callback( WP_REST_Request $request ) {
		...
		$data = [
			'gateways'               => [],
			'billingFields'          => $fields,
			'billingAddressDisabled' => Functions::is_billing_address_disabled(),
			'address'                => [
				'countries'      => rtcl()->countries->get_all_countries(),
				'states'         => rtcl()->countries->get_allowed_country_states(),
				'country_locale' => rtcl()->countries->get_country_locale()
			]
		];
		...
		if ( !empty( $gateways ) ) {
			foreach ( $gateways as $gateway ) {
				if ( 'yes' === $gateway->enabled ) {
					if ( "woo-payment" == $gateway->id ) {
						$data['gateways'] = $gateway->rest_api_data();
					} else {
						$data['gateways'][] = apply_filters( 'rtcl_rest_api_gateway_data', $gateway->rest_api_data(), $gateway );
					}
				}
			}
		}
