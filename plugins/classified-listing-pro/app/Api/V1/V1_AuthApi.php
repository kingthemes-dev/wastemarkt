	...
	public function register_routes() {
		...
		register_rest_route( 'rtcl/v1', 'social-login', [
			...
			'args'                => [
				...
				'first_name'                => [
					'type'        => 'string',
					'description' => 'Social login first name',
				],
				'last_name'                => [
					'type'        => 'string',
					'description' => 'Social login first name',
	...
	public function social_login_callback( WP_REST_Request $request ) {
		...
		switch ( $type ) {
			...
			case 'google_firebase':
				...
				$token_type = $request->get_param( 'firebase_token_type' );
				if ( 'id_token' === $token_type ) {
					$result = wp_remote_get( 'https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=' . $access_token );
				} else {
					$result = wp_remote_get( 'https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=' . $access_token );
				}
				...
				if ( $response_code !== 200 && ! empty( $g_user->error_description ) ) {
					$response = [
						'status'        => "error",
						'error'         => 'FORBIDDEN',
						'code'          => $response_code,
						'error_message' => $g_user->error_description
					];
				...
				$userData = [
					...
					'first_name' => !empty( $g_user->given_name ) ? $g_user->given_name : ( !empty( $request->get_param( 'first_name' ) ) ? trim( sanitize_text_field( wp_unslash( $request->get_param( 'first_name' ) ) ) ) : '' ),
					'last_name'  => !empty( $g_user->family_name ) ? $g_user->family_name : ( !empty( $request->get_param( 'last_name' ) ) ? trim( sanitize_text_field( wp_unslash( $request->get_param( 'last_name' ) ) ) ) : '' )
				];
				break;
			case 'apple':
				...
				\AppleSignIn\Vendor\JWT::$leeway = 60; //TODO: Patch added https://github.com/googleapis/google-api-php-client/issues/1630
