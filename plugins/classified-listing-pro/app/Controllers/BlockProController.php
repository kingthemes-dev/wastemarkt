...
class BlockProController {
	...
	public function save_block_css() {
		...
		try {
			if ( ! current_user_can( 'edit_posts' ) ) {
				throw new Exception( __( 'User permission error', 'classified-listing-pro' ) );
			}
			...
			if ( $post_id == 'rtcl-widget' && $params['has_block'] ) {
				...
				wp_send_json_success( [ 'message' => __( 'Widget CSS Saved', 'classified-listing-pro' ) ] );
			}
			...
			if ( $params['has_block'] ) {
				...
				if ( ! $wp_filesystem->put_contents( $dir . $filename, $block_css ) ) {
					wp_send_json_error( [ 'message' => __( 'CSS can not be saved due to permission!!!', 'classified-listing-pro' ) ] );
				}
				update_post_meta( $post_id, '_rtcl_block_css', $block_css );
				wp_send_json_success( [ 'message' => __( 'Css file has been updated', 'classified-listing-pro' ) ] );
			} else {
				...
				wp_send_json_success( [ 'message' => __( 'Data Delete Done', 'classified-listing-pro' ) ] );
			}
		} catch ( Exception $e ) {
	...
	public function appended( $server ) {
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_success( new WP_Error( 'rtcl_block_user_permission', __( 'User permission error', 'classified-listing-pro' ) ) );
		}
		global $wp_filesystem;
		if ( ! $wp_filesystem ) {
			require_once( ABSPATH . 'wp-admin/includes/file.php' );
		...
		if ( $post_id ) {
			...
			wp_send_json_success( [ 'message' => __( 'Data retrieve done', 'classified-listing-pro' ) ] );
		} else {
			wp_send_json_error( [ 'message' => __( 'Data not found!!', 'classified-listing-pro' ) ] );
		}
	}
	...
	public function get_posts_call() {
		...
		if ( isset( $post['postId'] ) ) {
			...
			wp_send_json_error( new WP_Error( 'rtcl_block_data_not_found', __( 'Data not found!!', 'classified-listing-pro' ) ) );
		}
	}
	...
	public function editor_assets_pro() {
		wp_enqueue_script( 'rtcl-gb-blocks-js-pro', rtclPro()->get_assets_uri( 'block/main.js' ),
			[ 'rtcl-gb-blocks-js','wp-block-editor', 'wp-blocks', 'wp-components', 'wp-element', 'wp-i18n' ], RTCL_PRO_VERSION, true );
