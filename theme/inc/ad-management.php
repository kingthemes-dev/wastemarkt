...
class Ad_Management {
	...
	public function __construct() {
		// Common
		add_action( 'classilist_header_top',     array( $this, 'header_top' ) );
		add_action( 'classilist_before_footer',  array( $this, 'footer' ) );
		add_action( 'classilist_before_sidebar', array( $this, 'before_sidebar' ) );
		add_action( 'classilist_after_sidebar',  array( $this, 'after_sidebar' ) );
		add_action( 'classilist_before_content', array( $this, 'before_content' ) );
		add_action( 'classilist_after_content',  array( $this, 'after_content' ) );

		// Listing Archive
		add_action( 'classilist_listing_before_items',  array( $this, 'listing_before_items' ) );
		add_action( 'classilist_listing_after_items',   array( $this, 'listing_after_items' ) );

		// Listing Single
		add_action( 'classilist_single_listing_before_contents', array( $this, 'single_listing_before_contents' ) );
		add_action( 'classilist_single_listing_after_contents',  array( $this, 'single_listing_after_contents' ) );
		add_action( 'classilist_single_listing_after_product',   array( $this, 'single_listing_after_product' ) );
		add_action( 'classilist_single_listing_after_location',  array( $this, 'single_listing_after_location' ) );
		add_action( 'classilist_single_listing_after_related',   array( $this, 'single_listing_after_related' ) );
	}
	...
	private function render_ad( $prefix, $class='' ) {
		if ( !RDTheme::$ad_options[$prefix.'_activate'] ) {
			return;
		}

		if ( RDTheme::$ad_options[$prefix.'_type'] == 'image' ) {
			$html = $this->get_image_ad( $prefix );
		}
		else {
			$html = RDTheme::$ad_options[$prefix.'_code'];
		}
		...
		$html = sprintf( '<div class="classilist-ad %1$s">%2$s</div>' , $class, $html );
		echo do_shortcode( $html );
	}

	private function get_page_type(){
		if ( class_exists( 'Rtcl' ) && Functions::is_listings() ) {
	...
	private function get_image_ad( $prefix ){
		if ( empty( RDTheme::$ad_options[$prefix.'_image']['id'] ) ) {
			return;
		}

		$img_html = wp_get_attachment_image( RDTheme::$ad_options[$prefix.'_image']['id'], 'full' );

		if ( !$img_html ) {
			return;
		}

		if ( !RDTheme::$ad_options[$prefix.'_url'] ) {
			$html = $img_html;
		}
		else {
			$attr = ' href="'.RDTheme::$ad_options[$prefix.'_url'].'"';
			if ( RDTheme::$ad_options[$prefix.'_newtab'] ) {
				$attr .= ' target="_blank"';
			}
			if ( RDTheme::$ad_options[$prefix.'_nofollow'] ) {
				$attr .= ' rel="nofollow"';
			}
